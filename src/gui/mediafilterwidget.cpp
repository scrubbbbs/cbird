...
MediaFilterWidget::MediaFilterWidget(QWidget* parent) : QWidget(parent) {
  ...
  constexpr struct {
    ...
    int value;
    int _padding;
  } flags[] = {{"All", MediaGroupTableModel::ShowAll, 0},
  ...
  while (flags[i].label) {
    ...
    if (_match & flags[i].value) action->setChecked(true);
    i++;
  }

  _menuButton = new QPushButton("Match...", this);

  connect(_menuButton, SIGNAL(clicked()), this, SLOT(matchButtonPressed()));
  ...
  connect(sizeFilter, SIGNAL(currentTextChanged(const QString&)), this,
          SLOT(sizeTextChanged(const QString&)));
  ...
  connect(pathFilter, SIGNAL(textChanged(const QString&)), this,
          SLOT(pathTextChanged(const QString&)));
...
void MediaFilterWidget::matchMenuTriggered() {
  ...
  for (QAction* a : _matchMenu->actions()) a->setChecked(_match & a->data().toInt());

  if (prev != _match) emit filterChanged(_match, _size, _path);
}
