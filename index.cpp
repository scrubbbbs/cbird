...
SearchParams::SearchParams() {
...

  {
    ...
    add({"alg", "Search algorithm", Value::Enum, counter++, SET_ENUM("alg", algo, values),
         GET(algo), GET_CONST(values), NO_RANGE});
  }

  {
    static const QVector<int> range{0, 65};
    add({"dht", "DCT hash distance threshold", Value::Int, counter++, SET_INT(dctThresh),
         GET(dctThresh), NO_NAMES, GET_CONST(range)});
  }

  {
    static const QVector<int> range{0, 100};
    add({"odt", "ORB descriptor distance threshold", Value::Int, counter++, SET_INT(cvThresh),
         GET(cvThresh), NO_NAMES, GET_CONST(range)});
  }
  ...
  add({"mn", "Minimum matches per needle", Value::Int, counter++, SET_INT(minMatches),
       GET(minMatches), NO_NAMES, GET_CONST(nonzero)});

  add({"mm", "Maximum matches per needle", Value::Int, counter++, SET_INT(maxMatches),
  ...
  add({"tm", "Enable template match result filter", Value::Bool, counter++, SET_BOOL(templateMatch),
       GET(templateMatch), NO_NAMES, NO_RANGE});
  ...
  add({"fg", "Filter Groups: remove duplicate groups from result: {a,b}=={b,a}", Value::Bool,
       counter++, SET_BOOL(filterGroups), GET(filterGroups), NO_NAMES, NO_RANGE});
  ...
  add({"mg", "Merge n-connected groups:", Value::Int, counter++, SET_INT(mergeGroups),
