...
class FileId {
 public:
  ...
  FileId(const QString& path) {
    auto handle = CreateFileW((wchar_t*)path.utf16(), GENERIC_READ, FILE_SHARE_READ,
...
class FileId {
 public:
  ...
  FileId() = delete;
  FileId(const QString& path) {
    if (stat(qUtf8Printable(path), &st) < 0)
      st.st_ino = 0;
...
Q_ALWAYS_INLINE uint qHash(const FileId& id) {
  return qHash(id.st.st_ino ^ id.st.st_dev);
}
#endif
...
static QString resolveJunction(const QString& path) {
  auto handle = CreateFileW((wchar_t*)path.utf16(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
  ...
  DWORD outSize = 0;
  auto ok = DeviceIoControl(handle, FSCTL_GET_REPARSE_POINT, NULL, 0,
                            buf, sizeof(buf), &outSize, NULL);
  ...
  auto name = QString::fromWCharArray(mp.PathBuffer + mp.PrintNameOffset / 2,
                                      mp.PrintNameLength / 2);
